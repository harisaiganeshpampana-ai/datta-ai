...
function applyMood(moodKey, save = true, isAuto = false) {
  // Check plan - mood system requires Shakti or higher
  const plan = localStorage.getItem('datta_plan') || 'free'
  const freePlans = ['free', 'arambh']
  const isCreator = localStorage.getItem('datta_is_creator') === 'true'
  
  if (freePlans.includes(plan) && !isCreator) {
    // Block mood for free users - show upgrade popup
...
function toggleMoodPanel() {
  ...
  const isCreator = localStorage.getItem('datta_is_creator') === 'true'
  if (freePlans.includes(plan) && !isCreator) {
    if (typeof showUpgradePopup === 'function') showUpgradePopup('mood')
    return
  }
